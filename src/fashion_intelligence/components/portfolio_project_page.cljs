(ns fashion-intelligence.components.portfolio-project-page
...
(defn find-project-by-slug [slug]
  (first (filter #(= slug (routes/title-to-slug (:title %))) data/projects-data)))

(defn portfolio-project-page []
  (let [selected-image-idx (r/atom nil)]
    (fn []
      (let [slug @routes/current-project-slug
            project (find-project-by-slug slug)]
        (if project
             ...
             [:div.max-w-6xl.mx-auto
              [:div.mb-8
               [:button.flex.items-center.text-gray-600.hover:text-gray-900.transition-colors.mb-6
                {:on-click #(routes/navigate-to :portfolio)}
          ...
          [:div.min-h-screen.flex.items-center.justify-center
           {:style {:background-image "url('/images/paper_texture.jpg')"
                   ...
                   :background-size "auto"}}
           [:div.text-center
            [:p.text-xl.text-gray-600.mb-4 "Proyecto no encontrado"]
            [:button.px-6.py-3.bg-gray-900.text-white.rounded.hover:bg-gray-800.transition-colors
             {:on-click #(routes/navigate-to :portfolio)}
             "Volver al Portfolio"]]])))))
